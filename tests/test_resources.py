...
class TestVolumes:
    ...
    async def test_attaches_to_an_instance_named_in_its_own_metro(self) -> None:
        recorder = platform(volumes=[volume(status="success")])
        async with client(recorder) as ukc:
            await ukc.volumes.get(name="data").attach(to=Ref(name="web", metro="FRA"), at="/data")
        assert json.loads(recorder.calls[-1].content) == [
            {"attach_to": {"name": "web"}, "at": "/data", "name": "data"}
        ]

    async def test_refuses_an_instance_named_in_another_metro(self) -> None:
        recorder = platform(volumes=[volume(status="success")])
        async with client(recorder) as ukc:
            with pytest.raises(ValueError, match="same metro"):
                await ukc.volumes.get(name="data").attach(to=Ref(name="web", metro="dal"), at="/d")
            with pytest.raises(ValueError, match="same metro"):
                await ukc.volumes.get(name="data").detach(from_=Ref(uuid="u9", metro="dal"))
        # Nothing was sent to `fra` that could have hit its own `web`.
        assert not [call for call in recorder.calls if call.method == "PUT"]

    async def test_refuses_an_instance_named_elsewhere_than_a_redirected_call(self) -> None:
        recorder = platform(volumes=[volume(status="success")])
        async with client(recorder) as ukc:
            # The call is redirected, so `fra` is no longer where it lands.
            with pytest.raises(ValueError, match="same metro"):
                await ukc.volumes.get(name="data", metro="fra").attach(
                    to=Ref(name="web", metro="fra"),
                    at="/d",
