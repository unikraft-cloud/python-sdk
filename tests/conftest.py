...
def missing(
    *, name: str | None = None, uuid: str | None = None, noun: str = "instance"
) -> dict[str, Any]:
    """One item of a response the API could not act on, as it reports it.

    A reference it does not hold comes back inside `data`, beside the ones it
    does: the response is still 200 and carries no top-level `errors`.
    """
    key, value = ("uuid", uuid) if uuid is not None else ("name", name)
    return {
        "status": "error",
        key: value,
        "message": f"No {noun} with {key} '{value}'",
        "error": 8,
    }
