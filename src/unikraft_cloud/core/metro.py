...
from .http import normalise_base_url
...
def metro_base_url(metro: Metro) -> str:
    ...
    if _URL_SCHEME.match(metro):
        return normalise_base_url(metro)
...
def metro_endpoint(metro: Metro) -> MetroEndpoint:
    ...
    Codes are lower-cased, as discovery reports them, so ``FRA`` and ``fra`` name
    the same place and tag results alike.
    """
    code = metro if is_url(metro) else metro.lower()
    return MetroEndpoint(metro=code, base_url=metro_base_url(code))
...
def same_metro(metro: Metro, endpoint: MetroEndpoint) -> bool:
    """Whether a metro names the place an endpoint serves.

    A metro is usually named by code (``fra``), but an explicitly configured
    endpoint is named by URL, so either spelling of the same place counts.
    """
    return metro.lower() == endpoint.metro.lower() or metro_base_url(metro) == endpoint.base_url
