...
class Remove:
    ...
    It travels as a ``set`` with no value, which is how the API clears one.
...
def to_patch_items(
    ...
    for prop, value in patch.items():
        ...
        if isinstance(value, Remove):
            # A `set` with no value clears the property. A bare `del` does not:
            # it removes nothing from a list or map, and a scalar rejects it.
            items.append(PatchItem(prop=prop, op="set"))
            continue
        items.append(
...
def check_props(changes: Sequence[PatchItem], allowed: tuple[str, ...], noun: str) -> None:
    """Reject a property this resource cannot change, before anything is sent."""
    unknown = sorted({item.prop for item in changes} - set(allowed))
    if unknown:
        raise TypeError(
            f"{unknown[0]!r} is not one of the properties an update can change on this {noun}. "
...
class ResourceEditor(Generic[R]):
    ...
    def apply(self) -> R:
        ...
        # The commit is lazy -- a handle that sends when awaited -- so hand it a
        # snapshot: a set() or add() after apply() must not alter what it sends.
        return self._commit(list(self._items))
