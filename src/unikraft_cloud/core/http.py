...
def default_timeout() -> httpx.Timeout:
    """A timeout of its own for a client that was given none.

    The Fetch-based JavaScript SDK sends no timeout at all, and the platform
    API's `wait` operations legitimately block for minutes. A read timeout would
    break them, so only connecting is bounded.
    """
...
class ApiClientConfig:
    ...
    timeout: float | httpx.Timeout | None = field(default_factory=default_timeout)
...
def normalise_base_url(base_url: str) -> str:
    """An API base URL the generated operation paths can be appended to.
...
#: Separator between two lines of one event (``\n``, ``\r\n`` or ``\r``). The
#: spec names these three only, unlike `str.splitlines()`, which would also break
#: a payload on the line separators JSON leaves unescaped (e.g. ``U+2028``).
...
def _sse_data(block: str) -> str | None:
    ...
    """
    values: list[str] = []
    for line in _SSE_LINE_BOUNDARY.split(block):
...
class CommaSeparated(list[str]):
    """A list the API reads as one parameter with comma-separated items.

    The API splits such a value on a literal comma before decoding it, so the
    separator must travel unescaped: ``?tags=a,b``, not ``?tags=a%2Cb``.
    """


def comma_separated(values: Sequence[str] | None) -> CommaSeparated | None:
    """Mark a list to be sent as one comma-separated parameter."""
    return None if values is None else CommaSeparated(values)


def encode_query(query: Mapping[str, QueryValue] | None) -> str | None:
    """Serialise a query mapping into a query string.

    A list becomes one repeated parameter per item, which is how the platform
    API spells its filters (``?uuid=a&uuid=b``). A :class:`CommaSeparated` list
    becomes one parameter instead, with its items joined by a literal comma.
    """
    if not query:
        return None
    pairs: list[str] = []
    for key, value in query.items():
        if value is None:
            continue
        name = _quote(key)
        if isinstance(value, CommaSeparated):
            if value:
                pairs.append(f"{name}={','.join(_quote(item) for item in value)}")
        # `str` is a Sequence, so it has to be excluded before the list case.
        elif isinstance(value, (list, tuple)):
            pairs.extend(f"{name}={_quote(_query_scalar(item))}" for item in value)
        else:
            pairs.append(f"{name}={_quote(_query_scalar(value))}")
    return "&".join(pairs) or None


def _quote(text: str) -> str:
    # Everything is escaped, commas included: only a separator stays literal.
...
def _lower_keys(headers: Mapping[str, str]) -> dict[str, str]:
    """Header names are case-insensitive; lower-casing them makes merging so too."""
...
class ApiClient:
    ...
    def _build_request(
        ...
        *,
        ...
        base = normalise_base_url(base_url) if base_url else self._base_url

        merged: dict[str, str] = {"accept": accept, **self._default_headers}
        if headers:
            merged.update(_lower_keys(headers))
        # The token is applied last: a caller's own `authorization` header would
        # otherwise silently deauthenticate the request. Names are lower-cased on
        # the way in, so `Authorization` is replaced rather than sent alongside.
        if self._config.token:
            merged["authorization"] = f"Bearer {self._config.token}"

        method = method.upper()
        content: bytes | None = None
        # A body is sent whatever the method. Most GET filters travel as query
        # parameters instead, but a few operations model an input only as a
        # body, and the API reads it from there.
        if body is not None:
            content = encode_body(body)
            merged["content-type"] = "application/json"

        # The query string is built here rather than by httpx, which would escape
        # the literal commas a comma-separated parameter needs.
        query_string = encode_query(query)
        return self._http.build_request(
            method,
            base + path + (f"?{query_string}" if query_string else ""),
            content=content,
            headers=merged,
    ...
    async def _send(self, request: httpx.Request, *, stream: bool) -> httpx.Response:
        if self._http.is_closed:
            raise UnikraftCloudError(
                "This client is closed, so it cannot send requests. Build another one.",
                kind="network",
            )
        try:
    ...
    def _network_error(self, request: httpx.Request, cause: httpx.HTTPError) -> UnikraftCloudError:
    ...
    def _raise_for_status(self, response: httpx.Response, parsed: Any) -> None:
        # Only 2xx is success. Redirects are not followed, so a 3xx that gets this
        # far is an answer the caller cannot use, whatever its body says.
        if not response.is_success:
            raise error_for_status(
                response.status_code,
