...
async def fanout_collect(
    endpoints: Sequence[MetroEndpoint],
    each: Callable[[MetroEndpoint], Awaitable[list[T]]],
) -> list[T]:
    """Concatenate what every metro returned, or raise with what did arrive.

    Metros that answered are reported even when others failed: the partial
    results travel on the raised error as ``err.results``, because an operation
    that returns a list cannot yield them as an iteration would.
    """
    outcomes = await fanout_settled(endpoints, each)
    results: list[T] = []
    failures: list[MetroFailure] = []
    for outcome in outcomes:
        ...
        # A metro can fail only in part; what it did finish is a result too.
        if isinstance(outcome.error, UnikraftCloudError):
            results.extend(outcome.error.results)
    if failures:
