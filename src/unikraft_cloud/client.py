...
from .api import Api
from .core.http import UNSET, ApiClientConfig, TimeoutOption, Unset, default_timeout
...
class Scope:
    ...
    def __repr__(self) -> str:
        # A pinned client covers its endpoint, whatever scope it was given.
        pinned = self._session.pinned
        return f"<{type(self).__name__} metros={(pinned.metro if pinned else self.scope)!r}>"
...
class UnikraftCloud(Scope):
    ...
    def __init__(
        ...
        *,
        ...
        :param timeout: Timeout applied to every request. Omitted, an injected
            `http` client keeps its own; otherwise the default bounds connecting
            but not reading, because the platform API's ``wait`` operations
            block for as long as they were asked to.
        """
        # An explicit endpoint settles where requests go, so the environment's
        # metro does not apply: it would name a different place.
        env_metro = None if base_url else os.environ.get("UKC_METRO")
        chosen_metro = metro or env_metro
        if chosen_metro and not is_url(chosen_metro):
            chosen_metro = chosen_metro.lower()
        ...
        # A metro named beside an explicit URL stays the identity of the place.
        # An endpoint named on its own has only its URL to go by.
        named = chosen_metro if chosen_metro and not is_url(chosen_metro) else None
        pinned_metro = named or resolved

        # An injected client keeps its own timeout; otherwise the default
        # applies. Every request carries the value, so it is settled here.
        if not isinstance(timeout, Unset):
            settled_timeout = timeout
        elif http is not None:
            settled_timeout = http.timeout
        else:
            settled_timeout = default_timeout()
        ...
        self._http = http or httpx.AsyncClient(
            transport=transport, trust_env=trust_env, timeout=settled_timeout
        )
        platform = ApiClientConfig(
            ...
            http=self._http,
            timeout=settled_timeout,
        )
        control_plane = platform.with_base_url(
            metro_base_url(control_plane_url or CONTROLPLANE_BASE_URL)
        )
        ...
        session = Session(
            SessionConfig(
                ...
                named_metro=named,
                pinned=(
                    MetroEndpoint(metro=pinned_metro, base_url=resolved) if explicit_url else None
                ),
            )
        )
    ...
    def metro(self, metro: Metro) -> MetroClient:
        ...
        A client pinned to one endpoint has only that metro to offer, so naming
        another one raises rather than quietly reaching the pinned one.
        ...
        """
        endpoint = self._session.endpoint_for(metro)
