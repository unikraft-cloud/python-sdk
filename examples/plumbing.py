...
from unikraft_cloud import ApiClientConfig, metro_base_url
...
async def main() -> None:
    ...
    async with httpx.AsyncClient() as http:
        ...
        # idiomatic layer does for you. The reported endpoint ends in `/v1`,
        # which the operation paths already carry, so normalise it first.
        ...
        for found in listed.data.metros if listed.data and listed.data.metros else []:
            if not found.endpoint:
                continue
            there = await instances.get_instances(count=1, base_url=metro_base_url(found.endpoint))
